...
**Pure Svelte + IBM Carbon Svelte. Every one of the 144 UI components touched exactly once and converted to: carbon-components-svelte components + carbon-icons-svelte + scoped Svelte `<style>` consuming `--cds-*` tokens + Carbon spacing/type. No Tailwind, no Lunaris token layer, no shadcn `ui/*`, no Geist/Fira.** Tailwind utility classes are removed *as part of each component's single migration pass* (not bridged) — so no component is touched twice and no throwaway glue is maintained.
