...
2. **Stores remain justified ONLY for "complex asynchronous data streams" or RxJS interop / manual subscription control** (verbatim: _"Stores are still a good solution when you have complex asynchronous data streams or it's important to have more manual control over updating values or listening to changes."_).
3. **Module-singleton `$state` is safe WITHOUT context when there is no SSR** (verbatim: _"If you're not using SSR (and can guarantee that you won't need to use SSR in future) then you can safely keep state in a shared module, without using the context API."_). Context is only needed to prevent cross-user SSR leakage.
