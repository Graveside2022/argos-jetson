...
- **0 components reference `--cds-*` tokens today** — and they *can't*, because of the prerequisite below.
...
> **P1 gate — must ship first:** switch `g100.css` → `all.css` and wrap the app in `<Theme theme="g100">` in `src/routes/+layout.svelte`. Until this lands, *no* `--cds-*` remediation works at all — any "fix" that swaps a Lunaris var for `--cds-text-secondary` will render transparent/black. This is non-negotiable ordering.
