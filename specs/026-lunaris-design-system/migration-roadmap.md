...
A11y audit + dead-code cleanup. **Engineering reasoning**: comprehensive WCAG 2.2 AA audit of Argos's 1736 frontend files / 408K LOC against Carbon's a11y baseline (now established by 5 phases of chassis adoption). Specific scope: focus order across migrated sites, color contrast for any custom views still using bespoke colors, keyboard navigation in legacy bespoke modals (the deferred Phase 4-5 follow-up sites), aria-* completeness on chassis consumers. Plus dead-code cleanup of bespoke implementations superseded by chassis (e.g., the `lib/components/ui/input/input.svelte` shadcn-svelte wrapper is now zero-consumer after Phase 6's DeviceToolbar migration). Branch: `feature/spec-026-carbon-phase-7-a11y-cleanup`. Effort: 2-5 days (depends on audit findings). After Phase 7 closes the main spine, the 5 deferred sub-phases (PanelStatus, TooltipIcon, Popover, ToolsFlyout restructure, EditorTabBar) bundle as a "Phase 8 — deferred cleanup" wrap-up.
