 ...
 *
 * Phase 3 / ADR-0003: migrated from `svelte/store` to the rune state layer.
 * The internal `writable` is replaced by `$state.raw` + `update`/`set` shims
 * (same signatures the persistence helpers + action creators expect); the
 * public API exposes `.current` instead of `.subscribe`. Action methods and
 * `getSnapshot()` are unchanged.
 */
...
function createScanLifecycleActions(update: StoreUpdate, getState: () => GSMEvilState) {
	return {
		startScan: () =>
			updateAndPersist(update, (state) => {
		...
		getAbortController: (): AbortController | null => {
			return getState().scanAbortController;
		}
	};
}
...
function createUtilityActions(update: StoreUpdate, set: StoreSet, getState: () => GSMEvilState) {
	return {
		...
		getSnapshot: (): GSMEvilState => getState()
	};
}

function createGSMEvilStore() {
	let value = $state.raw<GSMEvilState>(defaultState);
	const update: StoreUpdate = (updater) => {
		value = updater(value);
	};
	const set: StoreSet = (next) => {
		value = next;
	};
	const getState = (): GSMEvilState => value;
