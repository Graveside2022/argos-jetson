...
let reconDataValue = $state.raw<ReconData>(EMPTY);

let reconStatusValue = $state<ReconStatus>('idle');
export const reconStatus = {
	get current(): ReconStatus {
		return reconStatusValue;
	},
	set(value: ReconStatus): void {
		reconStatusValue = value;
	}
};

let reconErrorValue = $state<string | null>(null);
const reconError = {
	get current(): string | null {
		return reconErrorValue;
	},
	set(value: string | null): void {
		reconErrorValue = value;
	}
};

// ── Derived views (getters reading the runes state — reactive when read in
// a `.svelte`/`.svelte.ts` template/derived/effect) ───────────────────

export const reconTargets = {
	get current(): ReconTarget[] {
		return reconDataValue.targets;
	}
};
export const reconAlerts = {
	get current(): ReconAlert[] {
		return reconDataValue.alerts;
	}
};
export const weakSecurityTargets = {
	get current(): ReconTarget[] {
		return reconDataValue.targets.filter(
			(t) =>
				!t.encryption ||
				t.encryption === 'Open' ||
...
export const wpsTargets = {
	get current(): ReconTarget[] {
		return reconDataValue.targets.filter((t) => t.wps_enabled);
	}
};
export const hiddenNetworks = {
	get current(): ReconTarget[] {
		return reconDataValue.targets.filter((t) => t.cloaked || !t.ssid || t.ssid === 'Hidden');
	}
};
export const busyAPs = {
	get current(): ReconTarget[] {
		return reconDataValue.targets
...
export const gpsTracked = {
	get current(): ReconTarget[] {
		return reconDataValue.targets.filter((t) => t.gps_bounds != null);
	}
};
...
async function doReconFetch(url: string, signal: AbortSignal): Promise<void> {
	...
	reconDataValue = parseReconResponse(data as Record<string, unknown>);
