...
let gpStatusValue = $state<GlobalProtectStatus>(DEFAULT_STATUS);
export const gpStatus = {
	get current(): GlobalProtectStatus {
		return gpStatusValue;
	},
	set(value: GlobalProtectStatus): void {
		gpStatusValue = value;
	}
};

let gpOutputValue = $state.raw<string[]>([]);
export const gpOutput = {
	get current(): string[] {
		return gpOutputValue;
	},
